...
class Khafre:
    ...
    def decrypt_block(self, block: bytes) -> bytes:
        ...
        for i in range(self.n_rounds - 1, -1, -1):
            left, right = right, left
            right = rotl(right, ROTATION_SCHEDULE[i])
            left = xor(left, SBOX[right[-1]])
        
        pt = xor(left + right, self.key)
        return pt
    
    def decrypt(self, ct: bytes) -> bytes:
        pt = bytearray()
        for i in range(0, len(ct), 8):
            pt.extend(self.decrypt_block(ct[i:i+8]))
        pt = bytes(pt)
    ...
    def decrypt_partial(self, block: bytes, n: int) -> bytes:
        assert len(block) == 8
        block = xor(block, self.key)
        left, right = block[:4], block[4:]
...
def remote_encrypt(pt):
    pi.sendlineafter("Try my encryption >", pt.hex())
    pi.recvuntil("Result:")
    ct_bytes = bytes.fromhex(pi.recvline().strip().decode())[:-8]
    return ct_bytes

zero_enc = remote_encrypt(bytes([0, 0, 0, 0, 0, 0, 0, 0]))

hypot_key = bytearray([0, 0, 0, 0, 0, 0, 0, 0])

def find_byte_possibilities(n, idx, a):
    pt = bytearray([0, 0, 0, 0, 0, 0, 0, 0])
    pt[idx] = a

    ct = remote_encrypt(pt)

    possibilities = set()
    for x in range(256):
        hypot_key[idx] = x
        cipher = Khafre(hypot_key)
        ppt1 = cipher.decrypt_partial(zero_enc, n + 1)
        ppt2 = cipher.decrypt_partial(ct, n + 1)
        diff = xor(ppt1, ppt2)
        if diff == bytes([0, 0, 0, 0, 0, 0, 0, a]):
            possibilities.add(x)
...
# 4 5 6 7 3 0 1 2
# 0 1 2 3 4 5 6 7
order = [2, 6, 0, 4, 1, 5, 3, 7]

for n, idx in enumerate(order):
    possibilities = find_byte_possibilities(n, idx, 1) & find_byte_possibilities(n, idx, 2)
    assert len(possibilities) == 1
    real_byte = next(x for x in possibilities)
    hypot_key[idx] = real_byte
